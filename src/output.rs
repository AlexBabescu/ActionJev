...
pub fn markdown(report: &Report) -> String {
    let status = if report.dry_run { "Dry run. No model review performed." } else if !report.complete { "Review incomplete. A budget or coverage limit was reached." } else if report.scanned_files == 0 { "No eligible files to review." } else if report.actionable_count() > 0 { "Review complete. Potential issues need your attention." } else if !report.findings.is_empty() { "Review complete. Uncertain findings need human review." } else { "Review complete. No issues reported within this scope." };
    let repo = github_repository();
    let mut text = format!("{MARKER}\n## ActionJev review\n\n**{status}**\n\n| Files reviewed | Investigate | Uncertain | Files skipped |\n|---:|---:|---:|---:|\n| {} | {} | {} | {} |\n\nReviewed commit: `{}`\n\n", report.scanned_files, report.actionable_count(), report.findings.len() - report.actionable_count(), report.skipped.len(), report.head);
    ...
    if !report.findings.is_empty() {
        text.push_str("### Findings\n\n| Location | Concern | Impact / 4 | Decision |\n|---|---|---:|---|\n");
        for f in &report.findings {
            ...
            let row = format!("| {} | {} | {:.2} | {} |\n", location, escape(&f.mechanism.replace('_', " ")), f.severity, if f.actionable { "Investigate" } else { "Uncertain" });
    ...
    if !report.findings.is_empty() {
        text.push_str("\n<details>\n<summary>Finding scores and thresholds</summary>\n\n| File | Category | Support | Confidence |\n|---|---|---:|---:|\n");
        for f in &report.findings {
            let row = format!("| {} | {} | {:.3} | {:.3} |\n", escape(&f.path), escape(&f.dimension), f.evidence_probability, f.confidence);
            if text.len() + row.len() > 59_000 { text.push_str("\nAdditional scores omitted.\n"); break; }
            text.push_str(&row);
        }
        text.push_str(&format!("\nInvestigate requires support of at least {:.3}, confidence of at least {:.3}, and impact of at least 1.000. Other findings remain uncertain.\n\n</details>\n", report.thresholds["evidence"].as_f64().unwrap_or_default(), report.thresholds["confidence"].as_f64().unwrap_or_default()));
    }
...
mod tests {
    ...
    #[test] fn uncertain_calibration_result_is_not_presented_as_clean() {
        ...
        assert!(body.contains("Uncertain findings need human review"));
        assert!(!body.contains("No issues reported"));
        assert!(body.contains("| 1 | 0 | 1 | 0 |"));
        assert!(body.contains("███████░░░"));
        assert!(body.contains("| pages.py | correctness | 0.370 | 0.400 |"));
